...
(function($) {
    ...
    $(document).ready(function () {
        ...
        $('.block-blade .gradient').each(function() {
          $(this).closest('.block-blade').prepend(this);
        });
        $('.page-blade .gradient, .node-blades .gradient').each(function() {
...
jQuery('document').ready(function(){
	// Make full blade clickable if it contains a link button
	jQuery('.blade').each(function(key,blade) {
		console.log('blade',key,blade);
		var clickthrough = jQuery('div.field-name-field-clickthrough-url a',blade);
		if (clickthrough.length) {
			var clickUrl = jQuery(clickthrough).attr('href');
			jQuery(blade).on('click',function(){ window.location = clickthrough.attr('href'); });
			jQuery(blade).css('cursor','pointer');
			return;
		}
		var links = jQuery('a',blade);
		if (links && (links.length == 1) && links[0]) {
			var link = false;
			for (var i = 0; i < links.length; i++) {
				if (!link) {
					var url = jQuery(links[i]).attr('href');
			...
			if (link) {
				...
				jQuery(blade).on('click',function(e){
